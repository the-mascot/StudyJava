...
$(document).ready(function(){
        $("#viewCount").change(function() {
           ...
           location.href="<%=context%>/sw/mngBoardList.do?pageSize="+pageSize+"&pageNum="+pageNum;
       });
   });
</script>
...
<div class="view_count_parent">
	...
	<div style="text-align: center;">
		<c:if test='${startPage > blockSize}'>
			<a href='<%=context%>/sw/mngBoardList.do?pageNum=${startPage-blockSize}'>[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			<a href='<%=context%>/sw/mngBoardList.do?pageNum=${i}'>[${i}]</a>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			<a href='<%=context%>/sw/mngBoardList.do?pageNum=${startPage+blockSize}'>[다음]</a>
		</c:if>
	</div>
</section>
